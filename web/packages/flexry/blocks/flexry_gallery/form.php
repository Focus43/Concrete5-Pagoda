...
?>
	...
	<div id="flexryGallery" class="ccm-ui">
        <ul class="nav nav-tabs">
            <li class="active"><a data-tab="#tabPaneImages"><?php echo t('Gallery'); ?></a></li>
            <li><a data-tab="#tabPaneSettings"><?php echo t('Settings'); ?></a></li>
            <li><a data-tab="#tabPaneTemplates"><?php echo t('Template Options'); ?></a></li>
            <li id="flexryOptionsRight" class="pull-right">
                ...
                <button id="chooseImg" type="button" class="btn" title="Select multiple with checkboxes." data-method="<?php echo FlexryGalleryBlockController::FILE_SOURCE_METHOD_CUSTOM; ?>"><?php echo t('Add Images'); ?></button>
            </li>
        </ul>
        ...
        <div class="tab-content">
            ...
            <div id="tabPaneImages" class="tab-pane active">
                ...
                <div class="fileSourceMethod <?php if((int)$this->controller->fileSourceMethod === FlexryGalleryBlockController::FILE_SOURCE_METHOD_CUSTOM){ echo 'active'; } ?>" data-method="<?php echo FlexryGalleryBlockController::FILE_SOURCE_METHOD_CUSTOM; ?>">
                    <div class="dups-warning alert alert-warning"><?php echo t('The same image was added more than once; duplicates have been removed.'); ?> <button type="button" class="close">&times;</button></div>
                    <div id="selectionsContainer">
                        <div id="imageSelections">
                            <p><?php echo t('Hover images and'); ?>: <i class="icon-hand-up"></i> <strong><?php echo t('click'); ?></strong> <?php echo t('to edit'); ?>; <i class="icon-move"></i> <?php echo t('to reorder'); ?>; <i class="icon-minus-sign"></i> <?php echo t('to remove'); ?>. <button id="flexryClearAll" class="btn btn-mini btn-warning" type="button"><?php echo t('Clear All'); ?></button></p>
                ...
                <div class="fileSourceMethod <?php if((int)$this->controller->fileSourceMethod === FlexryGalleryBlockController::FILE_SOURCE_METHOD_SETS){ echo 'active'; } ?>" data-method="<?php echo FlexryGalleryBlockController::FILE_SOURCE_METHOD_SETS; ?>">
                    <div class="well">
                        <h3><?php echo t('Choose One Or More File Sets'); ?></h3>
                        <p><?php echo t('If more than one File Set is used, images will be ordered randomly.'); ?></p>
                        <select id="fileSetPicker" class="input-block-level" name="fileSetIDs[]" multiple data-placeholder="<?php echo t('Choose one or more File Sets'); ?>">
                        ...
                        <p class="muted" style="padding-top:4px;"><?php echo t('The advantage to using File Sets is that you can simply add one or more images to a set, or image sets, in the File Manager, and the gallery will automatically update with the images (instead of adding by hand using the custom gallery option).'); ?></p>
                    </div>
                </div>
            </div>
            ...
            <div id="tabPaneSettings" class="tab-pane">
                <div class="well">
                    <h3><?php echo t('Image Size Settings'); ?></h3>
                    <p><?php echo t('Thumbnails are generally used to display on the page, while the full size image may be shown after a user-triggered event (ie. click to view full size in a lightbox).'); ?></p>
                    <table class="table table-bordered">
                        <thead>
                            <tr>
                                <th></th>
                                <th><?php echo t('Thumbnail Size'); ?></th>
                                <th><label class="checkbox"><?php echo $formHelper->checkbox('fullUseOriginal', FlexryGalleryBlockController::FULL_USE_ORIGINAL_TRUE, $this->controller->fullUseOriginal); ?> <?php echo t('Use Original Image As Full Size?'); ?></label></th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td><strong><?php echo t('Max Width (px)'); ?></strong></td>
                            ...
                            <tr>
                                <td><strong><?php echo t('Max Height (px)'); ?></strong></td>
                            ...
                            <tr>
                                <td><strong><?php echo t('Crop To Fit?'); ?></strong></td>
                                <td><label class="checkbox"><?php echo $formHelper->checkbox('thumbCrop', FlexryGalleryBlockController::CROP_TRUE, $this->controller->thumbCrop); ?> <?php echo t('Yes'); ?></label></td>
                                <td><label class="checkbox"><?php echo $formHelper->checkbox('fullCrop', FlexryGalleryBlockController::CROP_TRUE, $this->controller->fullCrop); ?> <?php echo t('Yes'); ?></label></td>
                            </tr>
                        </tbody>
                    </table>
                    <p class="muted"><?php echo t('Images will be scaled to the maximum width/height, while maintaining the aspect ratio so as not to appear blurry.'); ?></p>
                </div>

                <div class="well">
                    <h3><?php echo t('Gallery Settings'); ?></h3>
                    <p><?php echo t('Allow user to click an image and view full size image overlayed on the page.'); ?></p>
                    <table class="table table-bordered">
                        <thead>
                            <tr>
                                <th colspan="5"><label class="checkbox" style="font-weight:normal;"><?php echo $formHelper->checkbox('lightbox[enable]', FlexryGalleryBlockController::LIGHTBOX_ENABLE_TRUE, (int) $this->controller->lightboxEnable, array('class' => 'enableLightboxCheckbox')); ?> <?php echo t('Enable Lightbox Gallery (If supported by template)'); ?></label></th>
                            </tr>
                        </thead>
                        <tbody class="flexry-lightbox-settings">
                            <tr>
                                <td rowspan="2"><strong><?php echo t('Mask'); ?></strong></td>
                                <td><?php echo t('Color'); ?></td>
                                <td><?php echo t('Opacity (&#37;)'); ?></td>
                                <td><?php echo t('Fade Speed (sec)'); ?></td>
                                <td><?php echo t('Click __ To Close'); ?></td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                <td rowspan="2"><strong><?php echo t('Animation'); ?></strong></td>
                                <td><?php echo t('Transition Effect'); ?></td>
                                <td colspan="3"><?php echo t('Duration (sec)'); ?></td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                <td rowspan="2"><strong><?php echo t('Display'); ?></strong></td>
                                <td><?php echo t('Captions'); ?> <?php echo $tooltips->generate('Title/Description', 'Responsive presets will trigger hiding on smaller devices.'); ?></td>
                                <td colspan="3"><?php echo t('Thumbnail Markers'); ?> <?php echo $tooltips->generate('Image Gallery', 'Displays circle icons (ie. pagination) to enable quick navigation of full gallery.') ?></td>
                            </tr>
                            <tr>
                ...
                <div class="well">
                    ...
                    <p><?php echo t('Fine-tune the block settings (mainly for advanced users).'); ?></p>
                    <table class="table table-bordered">
                        <tr>
                            <td><label class="checkbox"><?php echo $formHelper->checkbox('autoIncludeJsInFooter', FlexryGalleryBlockController::JS_IN_FOOTER_TRUE, $this->controller->autoIncludeJsInFooter); ?> <?php echo t('Output javascript includes in footer'); ?></label></td>
                        </tr>
                    </table>
                </div>
            </div>
            ...
            <div id="tabPaneTemplates" class="tab-pane">
                <div class="well">
                    <h3><?php echo t('Template Settings'); ?></h3>
                    <p><?php echo t('Configure different settings for different block templates.'); ?></p>
